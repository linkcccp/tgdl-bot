...
4. **分支合并核对**：检查该任务分支是否已 squash 合并到 dev（用 `git branch` 查看残留分支），未合并则要求该 agent 补做合并后再进入下一步；确认无残留未合并分支。**push origin / dev→main 合并绝不自动执行**，必须用户主动指示；合并到 main 后打 `v*` tag（tag 名对应版本号，如 `v2.4.0`）触发 CI 发布，agent 不自行发版。
...
- 全部代码须遵守 AGENTS.md 中的编码规范（标准注释 / SOLID / 设计模式 / 0 警告硬门槛）与 git 约定（dev 为汇聚分支、feat/fix/chore squash 合并到 dev、main 只接受 dev 合并的大版本，dev→main 合并由用户指示）。
