...
- `developer`：`src/TGBot` 全部功能模块实现（含 yt-dlp 集成、配置链路）
- `qa`：测试与验收
...
- 文档：`dotnet run --project tools/TgdlDocBuilder`（跨平台，替代已删除的 `build-docs.sh`；需 `dotnet tool install --global docfx`），输出 `docs/` 与 `docfx/api/`，须 0 警告（docfx `--warningsAsErrors`）。本机 docfx 需要 aspnetcore 运行时：`DOTNET_ROOT=$HOME/dotnet`（工具会自动清理 dotnet CLI 注入的 `DOTNET_ROOT_<ARCH>` 遮蔽变量，用户显式设置的 `DOTNET_ROOT` 始终生效）。`--help` 查看参数（`--skip-publish`/`--keep-cache`）。
