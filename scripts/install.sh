...
# ---------- 4. 可选：自动下载 telegram-bot-api（本地 Bot API Server） ----------
# 决策：本地 Bot API Server 是 Bot 正常工作的必需组件。tdlib 通过 GitHub releases
# 分发预编译二进制，但并非始终存在（当前其仓库无 release 资产，需从源码构建）。
# 因此：先尝试自动下载官方最新预编译二进制并校验 ELF；失败则回退为提示手动
# 安装（不阻断整体安装，deploy/install.sh 对缺失情况会给出手动下载提示）。
if [[ ! -x "$INSTALL_DIR/api/telegram-bot-api" ]]; then
    ...
    if curl -fsSL --retry 2 --retry-delay 2 --max-time 180 -o "$TMPDIR/deploy/telegram-bot-api" "$TBA_URL" \
        ...
        warn "telegram-bot-api 自动下载失败（tdlib 可能未发布预编译二进制）。"
        warn "Bot 依赖本地 Bot API Server 才能工作，请手动安装后重启："
        warn "  参考构建说明：https://tdlib.github.io/telegram-bot-api/build.html"
        warn "  或查看是否有预编译二进制：https://github.com/tdlib/telegram-bot-api/releases"
